...
if chat_or_standard is True:
    from services.models.siliconflow import deepseek_chat_stream
else:
    from services.models.siliconflow import deepseek_chat
...
def convert_command_values(commands):
    """根据 param_types 进行值类型转换"""
    for cmd in commands:
        device = cmd.get("device")
        action = cmd.get("action")
        if device in param_types:
            expected_type = param_types[device]
            cmd["action"] = convert_value_type(action, expected_type)
    return commands
