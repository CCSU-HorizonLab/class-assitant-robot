...
from .reporting_service import build_rule_report
from .sqlite_service import FileResultRepository
...
class PostgreSQLResultRepository:
    ...
    def _display_video(self, payload: Dict[str, Any], item: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Only fall back to a local demo video when the record is explicitly
        # flagged as a demo / smoke-test sample, or when its identifiers follow
        # the demo naming convention.  Otherwise we would silently show an
        # unrelated video for real classroom records that lack a video URL.
        analysis_id = (payload.get("analysis_id") or item.get("analysis_id") or "").lower()
        video_id_for_check = (video.get("video_id") or payload.get("video_id") or "").lower()
        is_demo = (
            self._bool_or_none(
                video.get("is_demo_playback_sample"),
                payload.get("is_demo_playback_sample"),
            ...
            or "cloud_playback_demo" in str(
                video.get("sample_type")
                or payload.get("sample_type")
                or capture.get("sample_type")
                or ""
            )
            or analysis_id.startswith("demo_")
            or video_id_for_check.startswith("demo_")
        )
        demo_video_url, demo_video_path = self._demo_video_url() if is_demo else ("", "")
        ...
        # When there is no cloud video URL yet, try to translate a local
        # raw_video_path that lives under the mounted captures directory into
        # a playable /captures/... URL.
        if not video_url and raw_video_path:
            local_path = Path(raw_video_path)
            captures_root = self.settings.video_local_captures_dir
            try:
                resolved_captures = captures_root.resolve()
                resolved_local = local_path.resolve()
                if resolved_local.is_file() and str(resolved_local).startswith(
                    str(resolved_captures)
                ):
                    relative = str(resolved_local.relative_to(resolved_captures)).replace(
                        "\\", "/"
                    )
                    video_url = f"/captures/{relative}"
            except (OSError, ValueError):
                pass  # path resolution failed — leave video_url empty

        # Fallback: search the uploads directory for a video file that matches
        # this record's analysis_id, or fall back to the only file if there is
        # exactly one video in the uploads directory.
        if not video_url:
            upload_dir = self.settings.video_upload_dir
            supported_suffixes = {".mp4", ".webm", ".mov", ".ogg"}
            try:
                if upload_dir.exists() and upload_dir.is_dir():
                    record_id = (
                        (payload.get("analysis_id") or item.get("analysis_id") or "").lower()
                    )
                    # 1. Exact prefix match: <analysis_id>__*.mp4
                    ...
                    if record_id:
                        for candidate in upload_dir.iterdir():
                            if (
                                ...
                                and candidate.stem.lower().startswith(record_id)
                            ):
                                matched = candidate
                                break
                    # 2. Single-file fallback: if only one video exists in uploads, use it
                    if matched is None:
                        all_videos = [
                        ...
                        if len(all_videos) == 1:
                            matched = all_videos[0]
                    if matched is not None:
                        video_url = f"/uploads/{matched.name}"
            except OSError:
                pass  # uploads directory unreadable — leave video_url empty
        if video_url:
    ...
    def _demo_video_url(self) -> Tuple[str, str]:
        supported_suffixes = {".mp4", ".webm", ".mov", ".ogg"}
        candidates = [
            self.settings.video_local_captures_dir,
            self.settings.video_upload_dir,
            self.settings.video_demo_dir,
        ]
        # Prefer any video file under the local captures directory so all
        # records share a single video source.
        for directory in candidates:
            if not directory.exists() or not directory.is_dir():
                continue
            preferred = directory / "video.mp4"
            if preferred.exists() and preferred.is_file():
                url = self._path_to_served_url(preferred, directory)
                if url:
                    return url, str(preferred)
        ...
        # Determine which candidate directory owns this file
        owner = next(
            (d for d in candidates if str(latest).startswith(str(d.resolve()))),
            candidates[0],
        )
        url = self._path_to_served_url(latest, owner)
        return (url, str(latest)) if url else ("", str(latest))

    def _path_to_served_url(self, file_path: Path, root_dir: Path) -> str:
        """Convert a local file path to a served URL based on which root it lives under."""
        try:
            resolved_root = root_dir.resolve()
            resolved_file = file_path.resolve()
            if not str(resolved_file).startswith(str(resolved_root)):
                return ""
            relative = str(resolved_file.relative_to(resolved_root)).replace("\\", "/")
            if resolved_root == self.settings.video_local_captures_dir.resolve():
