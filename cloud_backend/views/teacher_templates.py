...
def build_teacher_trends_html(current_user: Optional[dict] = None) -> str:
    ...
    <script>
      ...
      async function load(){
        ...
        const stage=payload.stage_distribution||{}; chart("stage-chart",{color:["#4f83ff","#14b8a6","#f59e0b","#94a3b8","#fb7185"],tooltip:{trigger:"item"},legend:{bottom:4,type:"scroll"},series:[{type:"pie",center:["50%","44%"],radius:["36%","62%"],avoidLabelOverlap:true,label:{formatter:"{b}\\n{d}%",fontSize:11},data:Object.entries(stage).map(([name,value])=>({name,value}))}]});
