...
from ..services import ResultRepository
...
<head>
  ...
  {PHASE31_STYLE}
  <style>
    .brand-card {{ background: linear-gradient(135deg, rgba(59, 130, 246, 0.08), rgba(255, 255, 255, 0.8) 52%, rgba(16, 185, 129, 0.08)); border-color: rgba(59, 130, 246, 0.15); }}
    .console-badge {{ display:inline-block; margin-bottom:10px; padding:6px 14px; border-radius:var(--radius-chip); background:var(--color-primary-soft); color:var(--color-primary-strong); font-weight:800; font-size:11px; letter-spacing:.08em; text-transform: uppercase; }}
    .analysis-layout {{ display: grid; grid-template-columns: minmax(0, 1fr) 340px; gap: 24px; align-items: stretch; min-width: 0; }}
    .video-box {{ min-height: 420px; border-radius: var(--radius-media); background: linear-gradient(145deg, #0b1329, #1e293b); color: #f8fafc; display: grid; place-items: center; padding: 20px; text-align: center; box-shadow: inset 0 0 0 1px rgba(255,255,255,0.05), 0 20px 40px -15px rgba(15, 23, 42, 0.2); }}
    .video-box.has-video {{ background: #000; padding:0; }}
    .video-box video {{ width: 100%; max-height: 430px; border-radius: var(--radius-media); background: #000; box-shadow: 0 10px 25px rgba(0,0,0,0.3); }}
    .video-placeholder-icon {{ font-size: 52px; margin-bottom: 16px; opacity: .55; }}
    .video-placeholder-title {{ font-size: 17px; font-weight: 700; margin-bottom: 6px; }}
    .video-placeholder-hint {{ font-size: 13px; opacity: .65; max-width: 320px; line-height: 1.6; }}
    .event-list {{ list-style: none; padding-left: 0; max-height: 360px; overflow-y: auto; display: flex; flex-direction: column; gap: 8px; scroll-behavior: smooth; }}
    .event-list li {{ border: 1px solid rgba(226, 232, 240, 0.8); border-radius: 12px; padding: 12px; cursor: pointer; background: #fff; font-size: 13px; transition: all 0.25s ease; }}
    .event-list li:hover {{ transform: translateX(4px); border-color: var(--brand); background: var(--color-primary-soft); color: var(--color-primary-strong); box-shadow: 0 2px 10px rgba(59,130,246,.06); }}
    .event-list li.active {{ border-color: var(--brand); background: var(--color-primary-soft); color: var(--color-primary-strong); font-weight: 600; box-shadow: 0 4px 16px rgba(59, 130, 246, 0.1); animation: pulse-glow 2s ease-in-out 1; }}
    .feedback-box {{ background: linear-gradient(135deg, rgba(59, 130, 246, 0.02), rgba(255, 255, 255, 0.7)); border: 1px solid rgba(59, 130, 246, 0.1); border-radius: 16px; padding: 18px; line-height: 1.7; }}
    .scope-note {{ margin-top: 12px; border: 1px solid #c7d2fe; background: #f8fbff; color: #334155; border-radius: 12px; padding: 14px; line-height: 1.65; font-size: 13px; }}
    ...
    .section-kicker {{ color: var(--brand); font-size: 12px; font-weight: 800; letter-spacing: .08em; text-transform: uppercase; margin: 0 0 6px; }}
    .score-strip {{ display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; margin: 12px 0; }}
    @keyframes countIn {{ from {{ opacity:0; transform:translateY(8px); }} to {{ opacity:1; transform:translateY(0); }} }}
    @keyframes pulse-glow {{ 0%,100% {{ box-shadow: 0 0 0 0 rgba(59,130,246,0); }} 50% {{ box-shadow: 0 0 0 8px rgba(59,130,246,.08); }} }}
    .score-card {{ border-radius: 14px; padding: 16px 14px; background: linear-gradient(180deg, #ffffff, rgba(239, 246, 255, 0.45)); border: 1px solid rgba(59, 130, 246, 0.1); transition: all 0.3s ease; position:relative; overflow:hidden; }}
    .score-card::before {{ content:''; position:absolute; top:0; left:0; right:0; height:3px; border-radius:3px 3px 0 0; background: linear-gradient(90deg, var(--brand), var(--color-secondary)); opacity:0; transition:opacity .3s; }}
    .score-card:hover {{ transform: translateY(-2px); border-color: rgba(59, 130, 246, 0.25); box-shadow: 0 8px 24px rgba(59, 130, 246, 0.08); }}
    .score-card:hover::before {{ opacity:1; }}
    .score-card strong {{ display: block; font-size: 26px; margin-top: 6px; color: var(--color-primary-strong); font-weight:800; letter-spacing:-.02em; animation: countIn .45s ease-out; }}
    .detail-box {{ background: #0f172a; color: #e2e8f0; border-radius: 12px; padding: 16px; white-space: pre-wrap; overflow: auto; max-height: 420px; border: 1px solid rgba(255,255,255,0.05); }}
    .filter-form {{ display:flex; gap:12px; flex-wrap:wrap; align-items:end; margin:16px 0 20px; }}
    .large-score {{ font-size: 56px; font-weight: 800; letter-spacing: -.03em; color: var(--color-primary-strong); line-height: 1.1; animation: countIn .5s ease-out; }}
    .chart-hero {{ min-height: 340px; }}
    @media (max-width: 860px) {{ .analysis-layout {{ grid-template-columns: 1fr; }} .score-strip {{ grid-template-columns:1fr; }} .large-score {{ font-size:40px; }} }}
  </style>
</head>
<body>
  <div class="page">
    ...
    <section id="chart-app" class="card analysis-card" data-marker="classroom-analysis-detail" data-initial-result-id="{selected_result_value}">
      ...
      <div class="analysis-layout">
        <div class="evidence-panel">
          ...
          <div id="video-area" :class="'video-box' + (video.status === 'playable' ? ' has-video' : '')" data-marker="video-area">
            <video v-if="video.status === 'playable'" id="classroom-video" controls :src="video.video_url"></video>
            <div v-else-if="video.status === 'pending'" class="video-placeholder">
              <div class="video-placeholder-icon">🎬</div>
              <p class="video-placeholder-title">课堂视频证据待接入</p>
              <p class="video-placeholder-hint">系统已检测到视频或采集信息，但当前没有可直接播放的云端视频地址。</p>
              <p class="muted" style="font-size:11px;opacity:.5;word-break:break-all" v-text="video.raw_video_path || video.video_id"></p>
            </div>
            <div v-else class="video-placeholder">
              <div class="video-placeholder-icon">📹</div>
              <p class="video-placeholder-title">暂无可播放视频</p>
              <p class="video-placeholder-hint">可先通过图表、事件和教学建议完成课堂复盘；视频格式处理不属于本阶段。</p>
            </div>
          </div>
    ...
    const chartApp = Vue.createApp({{
      ...
      methods: {{
        ...
        renderAttentionActivityTimeline() {{
          ...
          const series = [];
          if (showAttention) {{
            series.push({{ name: "专注度", type: "line", smooth: true, symbolSize: 6, symbol: "circle", lineStyle: {{ width: 3, shadowBlur: 8, shadowColor: "rgba(37,99,235,.25)" }}, areaStyle: {{ color: {{ type: "linear", x: 0, y: 0, x2: 0, y2: 1, colorStops: [{{ offset: 0, color: "rgba(37,99,235,.22)" }}, {{ offset: 1, color: "rgba(37,99,235,.01)" }}] }} }}, data: attention, markLine: {{ symbol: "none", lineStyle: {{ color: "#ea580c", type: "dashed", width: 2 }}, label: {{ formatter: "复盘阈值" }}, data: [{{ yAxis: 0.6 }}] }} }});
          }}
          series.push({{ name: "活跃度", type: "line", smooth: true, symbolSize: 6, symbol: "circle", lineStyle: {{ width: 3, shadowBlur: 8, shadowColor: "rgba(15,186,140,.25)" }}, areaStyle: {{ color: {{ type: "linear", x: 0, y: 0, x2: 0, y2: 1, colorStops: [{{ offset: 0, color: "rgba(15,186,140,.18)" }}, {{ offset: 1, color: "rgba(15,186,140,.01)" }}] }} }}, data: activity }});
          series.push({{ name: "提问候选", type: "scatter", symbolSize: 12, symbol: "pin", data: questionMarkers, tooltip: {{ valueFormatter: (value) => Array.isArray(value) ? value[2] : value }} }});
          ...
          chart.setOption({{
            ...
            tooltip: {{ trigger: "axis", backgroundColor: "rgba(15,23,42,.92)", borderColor: "rgba(255,255,255,.1)", textStyle: {{ color: "#f1f5f9", fontSize: 13 }} }},
            legend: {{ data: showAttention ? ["专注度", "活跃度", "提问候选"] : ["活跃度", "提问候选"], textStyle: {{ fontWeight: 600 }}, top: 6 }},
            grid: {{ left: 52, right: 28, bottom: 50, top: 56 }},
            ...
            xAxis: {{ type: "category", data: xAxis, axisLine: {{ lineStyle: {{ color: "#cbd5e1" }} }}, axisLabel: {{ hideOverlap: true, interval: "auto" }} }},
            yAxis: {{ type: "value", min: 0, max: 1, splitLine: {{ lineStyle: {{ color: "#f1f5f9" }} }}, axisLabel: {{ formatter: "{{value}}" }} }},
            series
          }});
        }},
        renderStageDistribution() {{
          ...
          chart.setOption({{
            ...
            animationEasing: "cubicOut",
            tooltip: {{ trigger: "item", backgroundColor: "rgba(15,23,42,.88)", borderColor: "rgba(255,255,255,.1)", textStyle: {{ color: "#f1f5f9" }} }},
            legend: {{ bottom: 4, left: "center", type: "scroll", textStyle: {{ fontWeight: 600, fontSize: 12 }}, itemGap: 14 }},
        ...
        renderZonePerformance() {{
          ...
          chart.setOption({{
            ...
            animationDuration: 600,
            animationEasing: "cubicOut",
            tooltip: {{ trigger: "axis", backgroundColor: "rgba(15,23,42,.88)", borderColor: "rgba(255,255,255,.1)", textStyle: {{ color: "#f1f5f9" }} }},
            legend: {{ data: showAttention ? ["专注度", "活跃度"] : ["活跃度"], textStyle: {{ fontWeight: 600 }}, top: 4 }},
            grid: {{ left: 48, right: 24, bottom: 44, top: 48 }},
            ...
            xAxis: {{ type: "category", data: zoneNames, axisLine: {{ lineStyle: {{ color: "#cbd5e1" }} }} }},
            yAxis: {{ type: "value", min: 0, max: 1.15, splitLine: {{ lineStyle: {{ color: "#f1f5f9" }} }} }},
            series
          }});
        }},
        renderEventDistribution() {{
          ...
          chart.setOption({{
            ...
            animationEasing: "cubicOut",
            tooltip: {{ trigger: "axis", backgroundColor: "rgba(15,23,42,.88)", borderColor: "rgba(255,255,255,.1)", textStyle: {{ color: "#f1f5f9" }} }},
            grid: {{ left: 48, right: 24, bottom: 82, top: 44 }},
            graphic: emptyGraphic(names.length === 0, "暂无事件数据"),
            xAxis: {{ type: "category", data: names, axisLabel: {{ rotate: 25, width: 90, overflow: "truncate", interval: 0, hideOverlap: true, fontWeight: 600, fontSize: 12 }}, axisLine: {{ lineStyle: {{ color: "#cbd5e1" }} }} }},
            yAxis: {{ type: "value", minInterval: 1, splitLine: {{ lineStyle: {{ color: "#f1f5f9" }} }} }},
            series: [{{
              name: "事件数",
              type: "bar",
              barMaxWidth: 42,
              itemStyle: {{ borderRadius: [8,8,0,0], color: {{ type: "linear", x: 0, y: 0, x2: 0, y2: 1, colorStops: [{{ offset: 0, color: "#fbbf24" }}, {{ offset: 1, color: "#f59e0b" }}] }} }},
              label: {{ show: true, position: "top", fontWeight: 700, color: "#92400e" }},
      ...
      computed: {{
        video() {{
          const rawVideo = (this.selectedDetail && this.selectedDetail.video) || {{status: "missing"}};
          if (!rawVideo.status || rawVideo.status === "missing") {{
            if (rawVideo.video_url) {{
              return {{...rawVideo, status: "playable"}};
            }}
            if (rawVideo.raw_video_path || rawVideo.video_id) {{
              return {{...rawVideo, status: "pending"}};
            }}
          }}
          return rawVideo;
        }},
